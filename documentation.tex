...
There are two main files: \texttt{hamiltonian.cu}, and \texttt{lanczos.cu}. The Hamiltonian generation is treated first. 
...
There are two codes used to generate the Hamiltonians: \texttt{testhamiltonian.cu} and \texttt{testhamiltonian.h}. The ``builder" function, \texttt{ConstructSparseMatrix}, creates and fills a number and three arrays with information about the Hamiltonian for a certain model and number of sites in the COO format (which stores the number of elements and three arrays: the values, the row indices, and the column indices):
